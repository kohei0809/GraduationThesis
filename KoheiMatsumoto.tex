...
\subsubsection{経路生成戦略}\label{route_strategy}
  ...
  関数\textsf{PLength}が呼び出された回数を比較して停止する手法(Deactivation with Count)と，
  エージェントの待機時間を比較して停止する手法(Deactivation with Time)の2つの手法を提案する．
  \\ \red{英語名については要検討}\\

  \subsubsection{Deactivation with Count}\label{sec:CountStop}
  ...
  \subsubsection{Deactivation with Time}\label{sec:TimeStop}
  \ref{sec:expAnalsis}項で詳細に説明するように，AMTDS/ERでは，イベント処理中心のグループと，
  エネルギー節約中心のグループの2グループに分かれる．
  Deactivation with Timeは，その性質とエージェントの待機時間を用いて停止するか否かを判断する手法である．
  \par
  ...
  図\ref{fig:flowchart_stop}は，Deactivation with Timeでの停止をするかどうかの判断のフローチャートである．
  ...
  \begin{table}
    ...
    \begin{minipage}[t]{.55\textwidth}
      ...
      \begin{tabular}{lcrr} \\ \hline
        種類 & パラメーター & 値 \\ \hline
        Deactivation with Count & $\DeactCheckInterval$ & 250,000 \\
                                & $\DeactCount$ & 100 \\
                                & $\DeactThreshold$ & 1.0 \\ \hline
        Deactivation with Time & $\DeactCheckInterval$ & 250,000 \\
                              & $\beta_s$ & 10 \\
                              & $\alpha_1$ & 0.1 \\
                              & $\alpha_2$ & 0.05 \\
                              & $S_{deact}$ & 0.7 \\ \hline
  ...
  この節では，\ref{sec:deactivation}節で述べたDeactivation with CountとDeactivation with Timeそれぞれの手法による，
  品質要求を満たす上で不要なエージェントを停止する実験を停止なしのAMTDS/ERと比較していく．

  \subsubsection{実験5: Deactivation with Countによる不要なエージェントの停止}\label{ex:ER5}
  \red{図はICAARTに使えるように英語にする．}\\
  この項では，\ref{sec:expAnalsis}での分析に基づき，
  \ref{sec:CountStop}項で述べたDeactivation with Countによる不要なエージェントの停止手法を評価するために，
  実験1と同じ設定で実験を行った．
  ...
  図内のDeactivation with CountというラベルがDeactivation with Countによる停止手法を用いたAMTDS/ERを示している．
  この実験では，停止したエージェントの数は6から8の間であり，ほとんどが7であった．
  ここで，$D(s)$と$C(s)$の値は小さい方が良いことに再度注意されたい．
  \par

  図\ref{fig:ds_CountStop}より，Deactivation with Countを用いたAMTDS/ERは，
  ...
  \subsubsection{実験6: Deactivation with Timeによる不要なエージェントの停止}\label{ex:ER6}
  \red{図はICAARTに使えるように英語にする．}\\
