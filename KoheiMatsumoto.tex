...
\subsubsection{経路生成戦略}\label{route_strategy}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.8\hsize]{figures/CorrectionScatter_Office_RandomStop_before.png}
    \caption{エージェントごとの$K^i$と待機時間の関係(ランダム停止時)}
    \label{fig:cscatter_RandomStop_before}
    \vspace{12pt}
    \centering
    \includegraphics[width=0.8\hsize]{figures/CorrectionScatter_Office_RandomStop_after.png}
    \caption{エージェントごとの$K^i$と待機時間の関係(ランダム停止後)}
    \label{fig:cscatter_RandomStop_after}
  \end{figure}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.8\hsize]{figures/CorrectionScatter_Office_DescendingStop_before.png}
    \caption{エージェントごとの$K^i$と待機時間の関係(降順停止時)}
    \label{fig:cscatter_DescendingStop_before}
    \vspace{12pt}
    \centering
    \includegraphics[width=0.8\hsize]{figures/CorrectionScatter_Office_DescendingStop_after.png}
    \caption{エージェントごとの$K^i$と待機時間の関係(降順停止後)}
    \label{fig:cscatter_DescendingStop_after}
  \end{figure}
  ...
  図\ref{fig:cscatter_RandomStop_before}は2,000,000ステップ時点での$K^i$と，
  ...
  次に，図\ref{fig:cscatter_RandomStop_after}は4,000,000ステップ時点での$K^i$と，
  3,000,000$\sim$4,000,000ステップでの総待機時間をプロットしたものである．
  図\ref{fig:cscatter_RandomStop_before}，\ref{fig:cscatter_RandomStop_after}より，
  品質要求を満たすために待機時間が短く，巡回中心であったエージェントが停止し，
  このままでは品質要求をみたせないため，その時は待機時間が長かったエージェントも$K^i$が小さくなり，
  待機時間も短くなったことが分かる．
  \par

  ここで，図\ref{fig:transition_randomStop}はエージェントの$K^i$の時間推移を示したものであり，
  ...
  図\ref{subfig:transition_random_10}より，停止後に品質要求が満たさなくなったと判断し，
  ...
  次に，図\ref{fig:cscatter_DescendingStop_before}は2,000,000ステップ時点での$K^i$と，
  1,000,000$\sim$2,000,000ステップでの総待機時間をプロットしたものであり，
  この時に停止するエージェントは赤でプロットしている．
  この図より，停止するエージェントは，$K^i$が大きいものから5体選べていることが分かる．
  また，図\ref{fig:cscatter_DescendingStop_after}は4,000,000ステップ時点での$K^i$と，
  3,000,000$\sim$4,000,000ステップでの総待機時間をプロットしたものである．
  図\ref{fig:cscatter_DescendingStop_before}，\ref{fig:cscatter_DescendingStop_after}より，
